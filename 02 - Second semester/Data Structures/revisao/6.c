...
int main() {
  int qtd;
  FILE *p;
  p = fopen("teste.txt", "ab");

  Cadastro *pessoas;
  printf("Digite a quantidade de pessoas cadastradas: ");
  scanf("%d", &qtd);
  ...
  // }
  for (int i = 0; i < qtd; i++) {
    fwrite(&pessoas[i], sizeof(Cadastro), 1, p);
  }
  ...
  fread(&teste, sizeof(Cadastro), 1, p);
  printf("%s\n%d\n%s\n\n", teste.nome, teste.idade, teste.endereco);
